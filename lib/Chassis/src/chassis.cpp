...
/**
 * Sets up a hardware timer on Timer4 to manage motor control on a precise
 * schedule.
 *
...
void Chassis::InitializeMotorControlTimer(void) {
  ...
  /**
   * Calculate TOP based on prescaler and loop duration. Note that loop is in
   * integer ms -- there may be some rounding. Multiples of 4 ms will be exact.
   */
  uint16_t top =
      ((CONTROL_LOOP_PERIOD_MS * 16000ul) >> 9) - 1; // divides by 512
...
/**
 * ISR for timing. On Timer4 overflow, we take a 'snapshot' of the encoder
 * counts and raise a flag to let the program it is time to execute the PID
 * calculations.
 */
ISR(TIMER4_OVF_vect) { Chassis::Timer4OverflowISRHandler(); }
