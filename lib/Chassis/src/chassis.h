...
protected:
   ...
   * overflows (see InitializeMotorControlTimer). Some of the calculations are
   * too lengthy for an ISR, so we set a flag and use that to key the
   * calculations.
   *
   * Note that we use in integer so we can see if we've missed a loop. If
   * loopFlag is more than 1, then we missed a cycle.
   */
