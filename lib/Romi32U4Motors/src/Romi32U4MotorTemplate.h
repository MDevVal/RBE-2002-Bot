...
protected:
   ...
   *
   * Calling this function will switch the control mode to DIRECT, if needed,
   * meaning speed control is lost.
   */
   ...
   * stores the change since the last call, which has units of "encoder
   * ticks/motor interval"
   *
   * We also use the function for zeroing the delta (for example, when switching
   * control modes), so interrupts are cleared when accessing encCount.
   */
  ...
  void SetTargetSpeed(float target) {
    ...
    if (ctrlMode != CTRL_SPEED) {
      ...
      // Also set prevCount to encCount so to avoid speed jumps when switching
      // mode
      CalcEncoderDelta();
    }
   ...
   * Chassis::UpdateMotors, which is on a timer schedule.
   */
  ...
  /**
   * InitializeMotorPWMTimer() should be called near the beginning of the
   * program. It sets up Timer4 to run at 38 kHz, which is used to both drive
   * the PWM signal for the motors and (tangentially) allow for a 38 kHz signal
   * on pin 11, which can be used, say, to drive an IR LED at a common rate.
   *
  ...
  static void InitializePWMTimerAndInterrupts(void) {
    ...
    ICR1 = 420;    // runs at 38kHz; lowers speed for given effort by 5% from
                   // Pololu version

    // set all three outputs to zero
    OCR1A = 0;
    OCR1B = 0;
    OCR1C = 0; // can be used to create 38 kHz signal on pin 11; must enable
               // output in TCCR1A above
...
protected:
  ...
  /**
   * SetEffort is used internally to set the motor effort without changing the
   * control mode.
   */
