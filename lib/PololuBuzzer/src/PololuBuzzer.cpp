...
static volatile unsigned int whole_note_duration =
    2000;                                    // the duration of a whole note
...
static volatile unsigned char
    staccato_rest_duration; // duration of a staccato rest,
...
ISR(TIMER4_OVF_vect) {
  if (buzzerTimeout-- == 0) {
    ...
    TC4H = top >> 8; // top 2 bits... (TC4H temporarily stores top 2 bits of
                     // 10-bit accesses)
...
void PololuBuzzer::init2() {
  ...
  TCCR4C =
      0x09; // bits 7 and 6 clear: normal port op., OC4A disconnected
            // bits 5 and 4 clear: normal port op., OC4B disconnected
  ...
  //    where TOP = OCR4C, OCR4D is updated at BOTTOM, TOV1 Flag is set on
  //    BOTTOM. OC4D is cleared on compare match when upcounting, set on compare
...
#else
  TCCR2A =
      0x21; // bits 7 and 6 clear: normal port op., OC4A disconnected
  ...
  //    on compare match when upcounting, set on compare match when
  //    downcounting; OC2A is disconnected.
...
void PololuBuzzer::playFrequency(unsigned int freq, unsigned int dur,
  ...
  while (top > 1023) {
    dividerExponent++;
    top = (unsigned int)((((F_CPU / 2 >> (dividerExponent)) * multiplier) +
                          (freq >> 1)) /
                         freq);
  }
#else
  unsigned int top;
  unsigned char newCS2 =
      2; // try prescaler divider of 8 first (minimum necessary for 10 kHz)
  ...
  while (top > 255) {
    ...
    top = (unsigned int)(((F_CPU / 2 / divider * multiplier) + (freq >> 1)) /
                         freq);
  }
#endif
  ...
  TCCR4B = (TCCR4B & 0xF0) |
           (dividerExponent +
            1);    // select timer 4 clock prescaler: divider = 2^n if CS4 = n+1
